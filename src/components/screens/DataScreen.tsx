...
import { PLACE_TYPES, GOVERNORATES, CITIES, DataOrder } from '@/types';
...
interface DataScreenProps {
  ...
  orders: DataOrder[];
  onRequestData: (placeName: string, placeType: string, governorates: string[], cities: string[]) => void;
  onDeleteOrderCity: (orderId: string, city: string, governorate: string) => void;
  governorates?: string[];
  cities?: Record<string, string[]>;
}

export function DataScreen({ availableData, onAddToMap, orders, onRequestData, onDeleteOrderCity, governorates: govProp, cities: citiesProp }: DataScreenProps) {
  const govList = govProp ?? GOVERNORATES;
  const cityMap = citiesProp ?? CITIES;
  ...
  const handleAvailableDataClick = (type: string) => {
    ...
    if (dataItem?.cities && dataItem.cities.length > 0) {
      ...
      availableCities.forEach((city) => {
        for (const [gov, govCities] of Object.entries(cityMap)) {
  ...
  const availableCities = useMemo(() => {
    ...
    selectedGovernorates.forEach((gov) => {
      const govCities = cityMap[gov] || [];
      cities.push(...govCities);
    });
    return cities;
  }, [selectedGovernorates, cityMap]);

  // Filter governorates by search
  const filteredGovernorates = useMemo(() => {
    if (!governorateSearch) return govList;
    return govList.filter(gov =>
      gov.toLowerCase().includes(governorateSearch.toLowerCase()) ||
      gov.includes(governorateSearch)
    );
  }, [governorateSearch, govList]);
  ...
  const getCitiesByGovernorate = useMemo(() => {
    ...
    return grouped;
  }, [selectedCities, selectedGovernorates, cityMap]);
  ...
  const handlePlaceOrder = () => {
    ...
    onRequestData(placeName, selectedType, [...selectedGovernorates], [...selectedCities]);
  ...
  const handleDeleteCity = () => {
    ...
    onDeleteOrderCity(orderId, city, governorate);
        ...
        {orders.length > 0 && (
          ...
          >
            ...
            >
              ...
              <div className="flex items-center gap-1.5 px-3 py-1.5 bg-orange-500/20 border border-orange-500/30 rounded-full shrink-0">
                ...
                <span className="text-xs font-medium text-orange-700 dark:text-orange-400">
                  {orders.filter(o => o.status === 'completed').length}/{orders.filter(o => o.status !== 'failed').length} مكتمل
                </span>
              </div>
            </button>
            ...
            <AnimatePresence>
              ...
              {orders.map((order) => {
                const typeInfo = PLACE_TYPES.find(t => t.value === order.placeType as string);
                      ...
                      <div className="absolute top-4 left-4 flex items-center gap-2">
                        <span className={`text-xs px-2 py-1 rounded-full shrink-0 ${
                          ...
                          {order.status === 'pending' ? 'قيد الانتظار'
                            : order.status === 'processing'
                            ? `${order.progress != null ? `${order.progress}% ` : ''}قيد المعالجة`
